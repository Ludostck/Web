window.onload = function() {
    ...
    console.log('Project name from URL:', projectName); // Log pour vérifier le nom du projet dans l'URL
    console.log('Pseudo from URL:', pseudo); // Log pour vérifier le pseudo dans l'URL
...
function loadFoldersAndFiles(projectName, pseudo) {
    console.log('Loading folders and files for project:', projectName); // Log pour vérifier le chargement des dossiers et fichiers

    fetch('rest/dossiers?pseudo=' + encodeURIComponent(pseudo) + '&projectName=' + encodeURIComponent(projectName))
    .then(response => {
        console.log('Received response from backend for folders and files'); // Log après réception de la réponse
        if (!response.ok) {
    ...
    .then(data => {
        ...
        data.folders.forEach(folder => {
            console.log('Adding folder to container:', folder.name); // Log chaque dossier ajouté au conteneur
